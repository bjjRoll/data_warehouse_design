...
BEGIN
    ...
    INSERT INTO
        staging.film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length,
                      replacement_cost, rating, last_update, special_features, fulltext, hub_film_hash_key,
                      film_static_hash_diff, film_dynamic_hash_diff, load_date, record_source)
    SELECT
        film_id,
        title,
        description,
...
BEGIN
    ...
    INSERT INTO
        staging.inventory (inventory_id, film_id, store_id, last_update, deleted, hub_inventory_hash_key,
                           hub_film_hash_key, link_film_inventory_hash_key, load_date, record_source)
    SELECT
        inventory_id,
        film_id,
        store_id,
...
BEGIN
    ...
    INSERT INTO
        staging.rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update, deleted,
                        hub_rental_hash_key, hub_inventory_hash_key, link_rental_inventory_hash_key, load_date,
                        record_source)
    SELECT
        rental_id,
        rental_date,
...
BEGIN
    INSERT INTO
        edw.hub_film (hub_film_hash_key, load_date, record_source, film_id)
    SELECT
        q.hub_film_hash_key,
        q.load_date,
        q.record_source,
...
BEGIN
    INSERT INTO
        edw.hub_inventory (hub_inventory_hash_key, load_date, record_source, inventory_id)
    SELECT
...
BEGIN
    INSERT INTO
        edw.hub_rental (hub_rental_hash_key, load_date, record_source, rental_id)
    SELECT
...
BEGIN
    INSERT INTO
        edw.link_film_inventory (link_film_inventory_hash_key, load_date, record_source, hub_film_hash_key,
                                 hub_inventory_hash_key)
    SELECT
...
BEGIN
    INSERT INTO
        edw.link_rental_inventory (link_rental_inventory_hash_key, load_date, record_source, hub_rental_hash_key,
                                   hub_inventory_hash_key)
    SELECT
...
BEGIN
    ...
    INSERT INTO
        edw.sat_film_static (hub_film_hash_key, load_date, load_end_date, record_source, hash_diff, title, description,
    ...
    WHERE
         sf.hub_film_hash_key IS NULL
      OR sf.hash_diff <> f.film_static_hash_diff;

    -- Помечаем неактуальные строки
    WITH
        updated_sat AS (SELECT
    ...
    FROM
        updated_sat us
    WHERE
          sf.hub_film_hash_key = us.hub_film_hash_key
      AND sf.load_date = us.load_date;

    -- Помечаем удаленные строки
    WITH
        deleted_sat AS (SELECT
                        ...
                        WHERE
                              f.hub_film_hash_key IS NULL
                          AND sf.load_end_date IS NULL)
    UPDATE edw.sat_film_static AS sf
    SET
    ...
    FROM
        deleted_sat ds
    WHERE
          sf.hub_film_hash_key = ds.hub_film_hash_key
      AND sf.load_date = ds.load_date;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    -- Добавляем новые строки по фильмам
    INSERT INTO
        edw.sat_film_dynamic (hub_film_hash_key, load_date, load_end_date, record_source, hash_diff, rental_duration,
    ...
    WHERE
         sf.hub_film_hash_key IS NULL
      OR sf.hash_diff <> f.film_dynamic_hash_diff;
