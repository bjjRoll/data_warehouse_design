...
AS $$
	BEGIN
		...
		INSERT INTO staging.address (address_id, address, district, city_id, postal_code, phone, last_update)
		SELECT 
			address_id, 
			address, 
			district, 
...
AS $$
	BEGIN
		...
		INSERT INTO staging.staff (staff_id, first_name, last_name, address_id, email, store_id, username, last_update, deleted)
		SELECT 
			staff_id,
			first_name,
			last_name,
...
AS $$
	BEGIN
		...
		INSERT INTO staging.store (store_id, manager_staff_id, address_id, last_update)
		SELECT 
			store_id,
			manager_staff_id,
			address_id,
...
AS $$
	BEGIN
		...
		INSERT INTO staging.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date, deleted, last_update)
		SELECT 
			payment_id,
			customer_id,
			staff_id,
...
AS $$
	BEGIN
		...
		INSERT INTO ods.film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length, replacement_cost, rating, last_update, special_features, fulltext)
		SELECT 
			film_id, 
			title, 
			description, 
...
AS $$
	BEGIN
		...
		INSERT INTO ods.inventory (inventory_id, film_id, store_id, last_update, deleted)
		SELECT 
			inventory_id, 
			film_id, 
			store_id,
...
AS $$
	BEGIN
		...
		INSERT INTO ods.address (address_id, address, district, city_id, postal_code, phone, last_update)
		SELECT 
			address_id, 
			address, 
			district, 
...
AS $$
	BEGIN
		...
		INSERT INTO ods.staff (staff_id, first_name, last_name, address_id, email, store_id, username, last_update, deleted)
		SELECT 
			staff_id,
			first_name,
			last_name,
...
AS $$
	BEGIN
		...
		INSERT INTO ods.store (store_id, manager_staff_id, address_id, last_update)
		SELECT 
			store_id,
			manager_staff_id,
			address_id,
...
AS $$
	BEGIN
		...
		WHERE odp.payment_id IN (
			SELECT 
				sp.payment_id 
			FROM staging.payment sp
		);
	
		INSERT INTO ods.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date, deleted, last_update)
		SELECT 
			payment_id,
			customer_id,
			staff_id,
...
AS $$
	BEGIN
		...
		INSERT INTO ods.rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update, deleted)
		SELECT 
			rental_id, 
			rental_date, 
			inventory_id, 
...
AS $$
	BEGIN
		...
		INSERT INTO integ.film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length, replacement_cost, rating , last_update, special_features, fulltext)
		SELECT 
...
AS $$
	BEGIN
		...
		INSERT INTO integ.inventory (inventory_id, film_id, store_id, last_update, deleted)
		SELECT 
...
AS $$
	BEGIN
		...
		INSERT INTO integ.address (address_id, address, district, city_id, postal_code, phone, last_update)
		SELECT 
...
AS $$
	BEGIN
		...
		INSERT INTO integ.staff (staff_id, first_name, last_name, address_id, email, store_id, username, last_update, deleted)
		SELECT 
...
AS $$
	BEGIN
		...
		INSERT INTO integ.store (store_id, manager_staff_id, address_id, last_update)
		SELECT 
			rsr.store_sk   AS store_id,
...
AS $$
	...
	BEGIN
		...
		INSERT INTO integ.payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date, deleted, last_update)
		SELECT 
...
AS $$
	...
	BEGIN
		...
		-- Вставляем в integ слой все созданные, удаленные или измененные строки с предыдущей загрузки
		INSERT INTO integ.rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update, deleted)
		SELECT 
			rr.rental_sk    AS rental_id,
			r.rental_date,
			ri.inventory_sk AS inventory_id,
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_film (film_id, title, description, release_year, language_id, rental_duration, rental_rate, length, replacement_cost, rating, special_features, fulltext, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			f.film_id, 
			f.title, 
			f.description, 
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_inventory (inventory_id, film_id, store_id, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			i.inventory_id,
			i.film_id,
			i.store_id,
		...
		WHERE 
			idl.inventory_id = i.inventory_id AND 
			i.is_active IS TRUE;
		...
		WHERE 
			i.inventory_id = di.inventory_id AND 
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_address (address_id, address, district, city_id, postal_code, phone, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			a.address_id, 
			a.address, 
			a.district, 
		...
		WHERE 
			adl.address_id = a.address_id AND 
			a.is_active IS TRUE;
		...
		WHERE 
			a.address_id = da.address_id AND 
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_city (city_id, city, country_id, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			c.city_id, 
			c.city, 
			c.country_id,
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_staff (staff_id, first_name, last_name, address_id, email, store_id, username, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			s.staff_id, 
			s.first_name, 
			s.last_name, 
...
AS $$
	BEGIN
		...
		INSERT INTO dds.dim_store (store_id, manager_staff_id, address_id, date_effective_from, date_effective_to, is_active, hash)
		SELECT 
			s.store_id,
			s.manager_staff_id,
			s.address_id,
...
AS $$	
	...
	BEGIN
		...
		-- Вставляем в integ слой все созданные, удаленные или измененные строки с предыдущей загрузки
		INSERT INTO dds.fact_payment (payment_id, customer_id, staff_id, rental_id, amount, payment_date, deleted, last_update)
		SELECT 
			p.payment_id, 
			p.customer_id, 
			p.staff_id, 
...
AS $$	
	...
	BEGIN
		...
		-- Вставляем в integ слой все созданные, удаленные или измененные строки с предыдущей загрузки
		INSERT INTO dds.fact_rental (rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update, deleted)
		SELECT 
			r.rental_id,
			r.rental_date,
			r.inventory_id,
...
AS $$
	BEGIN 
		...
		INSERT INTO mart.sales_by_date (sales_date_id, sales_date_title, amount)
		SELECT
...
CREATE OR REPLACE FUNCTION mart.get_data_from_sales_by_date() RETURNS TABLE(sales_date_id int, sales_date_title varchar, amount float)
AS $$
	SELECT 
		sales_date_id, 
		sales_date_title, 
...
CREATE OR REPLACE FUNCTION mart.get_data_from_sales_by_film() RETURNS TABLE(film_title varchar, amount float)
AS $$
	SELECT 
		film_title, 
		amount
...
CALL mart.fill_calendar('2005-01-01'::date, 7305);  -- Календарь до конца 2024 года
--------- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ------------
